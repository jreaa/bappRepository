...
$tblcita_presencial = DB::table('tblcita_presencial')->select('id','date','time','email','place','id_postulante','status')->get();
				    ...
				    <tr>
					  ...
					  <tbody id="tbody">
					    ...
					    <tr>
					      ...
					      <td><?php echo strtoupper($row['place']);?></td>
					      <td><?php echo $rutp; ?></td>
					      <td><?php echo strtoupper($namep);?></td>
					      <td><?php echo strtoupper($row['email']);?></td>
					      <td><?php echo strtoupper($row['status']);?></td>
					      <td><?php 
				          $status = json_decode(json_encode(DB::table('status_entrevista')->where('id_entrevista', '=', $row['id'])->get()), True);
					      foreach($status as $s){
					          $id_status = $s['id'];
					      }
					      ...
					      if($status){
					            echo "<a href='comentariosPresencial.php?id=$id_status'>Ver Comentarios</a>";
					      }else{
				       ...
				       ?></td>
					      <td>
					      	<a href="calificarEntrevistaPresencial.php?id=<?php echo $row['id'];?>&id_postulante=<?php echo $row['id_postulante'];?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pencil" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
...
<script type="text/javascript">
	$(document).ready(function(){
		$('#btn1').click(function(){
			$('#tbody').empty()
			let status = $('#btn1').val();
			let link = 'layouts/Entrevista/statusPresencial.php?status='
...
<script type="text/javascript">
	function deleteUser(id){
		if(confirm('¡Estas seguro que deseas eliminar la cita seleccionada?')){
			window.location="app/Controllers/ControllerDeleteCitaPresencial.php?id="+id;
		}
	}
</script>