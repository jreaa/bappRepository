...
$test_achs = DB::table('seguimiento_mantencion')->get();
			...
			<div class="row">
				<div class="col-md-12">
					<div class="head_section">
						<h3>Seguimiento Mantencion Siniestro</h3>
						<a href="mantencion_siniestro.php" class="btn btn-bapp" style="padding:12px">Nuevo</a>
						</div>
					</div>
				</div>
			</div>
								...
								<table class="table table-hover">
									...
									<thead>
										<tr>
											...
											<th scope="col">Llevo el vehiculo al taller</th>
											<th scope="col">Entrego el vehiculo al taller</th>
											<th scope="col">Ingreso el vehiculo al taller</th>
											<th scope="col">Recibio el vehiculo al taller</th>
									...
									foreach($array as $row) { ?>
										...
										<tbody>
											<tr>
											    ...
											    $m = json_decode(json_encode(DB::table('mantencion_siniestro')->where("id", "=", $row['id_mantencion'])->get()),True);
											    foreach($m as $c){
											        $patente = $c['patente'];
											    }
												...
												<td><?php echo $row['e_taller'];?></td>
												<td><?php echo $row['e_vehiculo'];?></td>
												<td><?php echo $row['i_taller'];?></td>
												<td><?php echo $row['r_conductor'];?></td>
												<td><button onclick="modal(<?php echo $row['id']; ?>)" class="btn btn-bapp" id="btnModal">Ver mantencion</button></td>
												<input type="hidden" id="modal" value="<?php echo $row['id'];?>">
												<td>
													 <button id="btn" class="btn btn-danger" onclick="deleteUser(<?php echo $row['id'];?>, 'Esta seguro que desea eliminar esta mantencion?', 'app/Controllers/controllerDeleteMantencion.php',<?php echo $privi;?>)"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-trash" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0V6z"/>
